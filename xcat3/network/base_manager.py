...
"""Base service manager functionality."""
...
class BaseServiceManager(object):
    def __init__(self, host, topic):
        super(BaseServiceManager, self).__init__()
        if not host:
            host = CONF.host
        self.host = host
        self.topic = topic
    ...
    def init_host(self, admin_context=None):
        ...
        :raises: RuntimeError when network service is already running.
        """
        if self._started:
        ...
        if (len(self._periodic_task_callables) >
                CONF.network.workers_pool_size):
            LOG.warning(_LW('This network service has %(tasks)d periodic tasks'
                            ' enabled, but only %(workers)d task workers '
                            'allowed by [network]workers_pool_size option'),
                        {'tasks': len(self._periodic_task_callables),
                         'workers': CONF.network.workers_pool_size})
        ...
        try:
            ...
            self.service = objects.Service.register(
                admin_context, self.host, type=self.type)
            ...
            LOG.warning(
                _LW(
                    "A service with hostname %(hostname)s type %(type)s was "
    ...
    def del_host(self, deregister=True):
        # Service deregistration fails if called on non-initialized
    ...
    def _service_record_keepalive(self):
        while not self._keepalive_evt.is_set():
            try:
                ...
                LOG.warning(_LW('Network service could not connect to database'
                                ' while heartbeating.'))
